...
public class SystemUtil extends BaseDAO<Object, Long> implements ISystemUtil {
	...
	public DispatchList findDispatchListByDlId(Long dlId) throws MyException {
		if (MyMatcher.isEmpty(dlId)) {
			throw new MyException(AppConstant.A002);
		}
		String hql = "from DispatchList where dlId = ?";
	...
	@Override
	public SysPositions findSysPositionsByUId(Long UId) throws MyException {
		if (MyMatcher.isEmpty(UId)) {
			throw new MyException(AppConstant.A004);
		}
		String hql = "from SysPositions where PId =(select PId from SysEmployee where ESn = (select ESn from LoginUser where UId = ?))";
		return super.findUnique(hql, UId);
	}

	@Override
	public LoginUser findLoginUserByUId(Long UId) throws MyException {
		if (MyMatcher.isEmpty(UId)) {
			throw new MyException(AppConstant.A004);
		}
		String hql = "from LoginUser where UId = ?";
		return super.findUnique(hql, UId);
	}

	@Override
	public SysEmployee findSysEmployeeByUId(Long UId) throws MyException {
		if (MyMatcher.isEmpty(UId)) {
			throw new MyException(AppConstant.A004);
		}
		String hql = "from SysEmployee where ESn = (select ESn from LoginUser where UId = ?)";
		return super.findUnique(hql, UId);
	}

	@Override
	public Boolean checkPermissionsByUIdAndDlId(String UId, Long dlId) throws MyException {
		if (MyMatcher.isEmpty(UId)) {
			throw new MyException(AppConstant.A004);
		}
		if (MyMatcher.isEmpty(dlId)) {
		...
		if (MyMatcher.isEmpty(dr)) {
			String hql = "from SysPositions where PId = (select PId from SysEmployee where ESn = (select ESn from LoginUser where UId = ?))";
			SysPositions sp = super.findUnique(hql, UId);
			if (MyMatcher.isEmpty(sp)) {
				return false;
			} else {
				if (sp.getPId() == AppConstant.EMPLOYEE) {
					return true;
				}
			}
		}
		return dr.getCheckNext().equals(UId);
	}
...
//		Long UId = 1L;
		...
		/* 查询指定报销单当前审批记录 */
		// DispatchResult dr = is.findCurrentDispatchResultByDlId(dlId);
		...
		/* 查找指定报销单所有流程 */
		// List<DispatchResult> dds = is.findDispatchResultByDlId(dlId);
		// for (DispatchResult dispatchResult : dds) {
		// System.out.println(dispatchResult.getDrId());
		// }

		/* 根据用户编号查询雇员 */
		// SysEmployee se = is.findSysEmployeeByUId(UId);
		// System.out.println(se.getEId());

		/* 通过用户 ID 查找指定用户职位 */
		// SysPositions sp = is.findSysPositionsByUId(UId);
		// System.out.println(sp.getPId());

		/* 通过用户 ID 查找用户 */
		// LoginUser lu = is.findLoginUserByUId(UId);
		// System.out.println(lu.getUId());
		...
		Boolean bl = is.checkPermissionsByUIdAndDlId("10000003", dlId);
